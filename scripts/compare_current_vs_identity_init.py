...
class IdentityInitResidualIOMinimalArchModel(MinimalArchModel):
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compare current initialization against identity initialization for one architecture."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=os.path.join("outputs", "toy_diffusion", "init_compare", "current_vs_identity_m6a2"),
    )
...
def pick_device(name: str) -> torch.device:
    ...
    if torch.cuda.is_available():
        return torch.device("cuda")
    print("[compare_current_vs_identity_init] cuda not available, using cpu")
    return torch.device("cpu")
...
def build_model(args: argparse.Namespace, device: torch.device, init_mode: str) -> MinimalArchModel:
    ...
    if init_mode == "current":
        return MinimalArchModel(cfg).to(device)
    if init_mode == "identity":
        return IdentityInitResidualIOMinimalArchModel(cfg).to(device)
...
def train_one_run(
    ...
    *,
    ...
    print(f"[compare_current_vs_identity_init] mode={init_mode} seed={torch_seed} step=-1 val_loss={init_val:.6f}")
    ...
    for step in range(args.max_steps):
        ...
        if step % args.log_every == 0 or step == args.max_steps - 1:
            ...
            print(
                f"[compare_current_vs_identity_init] mode={init_mode} seed={torch_seed} "
...
def make_plot(
    ...
    *,
    ...
    colors = {"current": "#1f77b4", "identity": "#d62728"}
    ...
    for summary in summaries:
        ...
        train_x = [row["step"] for row in train_rows]
        train_y = [math.log10(row["train_loss"]) for row in train_rows]
        val_x = [row["step"] for row in val_rows]
        val_y = [math.log10(row["val_loss"]) for row in val_rows]

        label = f"{init_mode} seed={seed}"
        train_ax.plot(train_x, train_y, color=color, linewidth=1.5, alpha=0.75, label=label)
        val_ax.plot(val_x, val_y, color=color, linewidth=1.5, alpha=0.75, label=label)
    ...
    fig.suptitle("Current Init vs Identity Init on M-M-M-M-M-M-A-A")
    fig.tight_layout(rect=(0, 0, 1, 0.9))
    plot_path = out_dir / "current_vs_identity_log_curves.png"
...
def main() -> None:
    ...
    seeds = choose_seeds(args.seed_selector, args.seed_count, args.seed_pool_max)
    ...
    summaries: list[dict[str, object]] = []
    for init_mode in ("current", "identity"):
        for seed in seeds:
    ...
    grouped = {
        "current": [row for row in summaries if row["init_mode"] == "current"],
        "identity": [row for row in summaries if row["init_mode"] == "identity"],
    }
    aggregate = {
        "architecture_id": args.architecture_id,
        "architecture_code": canonicalize_architecture_code(args.architecture_code),
    ...
    print(f"[compare_current_vs_identity_init] done output_dir={out_dir}")
    print(f"[compare_current_vs_identity_init] plot={plot_path}")
